...
=== Other than Windows Subsystem for Linux 1 ===

Due to the lack of critical packages building on MinGW is not
supported at this time.

Due to a namespace conflict building on CyGWin is not supported
at this time.

WSL 2 claims not to support USB or serial at this time.

No build files are available for Visual Studio at this time.

=== About WSL 1 ===

WSL 1 is  a component of Microsoft
Windows that implements an alternate kernel. That Linux
...
Icon in the taskbar.
2. Click on the search icon (it is a spyglass).
...
Due to WSL1 limitations, it is recommended at this time that you
only install the following packages or $distro equivalent.

----
Python
SCons (preferably 3.0+)
ncurses-dev (for build cgps and gpsmon)
asciidoctor and xmlto (to build the documentation)
----

Optionally, the following packages might also be installed
----
pyserial (for fine control of UBlox GPS and GREIS devices)
gnuplot (to graph gpsprof data)
libusb-dev (for old Garmin GPS receivers)
git (if building from the git repository)
----

=== Building on WSL ===

1. Get the list of packages from the appropriate section above.
2. If somehow you have acquired these instructions without the accompanying source code acquire it from one of the following locations.
  *  link:https://download-mirror.savannah.gnu.org/releases/gpsd/[gpsd \
release tarballs] (recommended)
  *  link:https://gitlab.com/gpsd/gpsd[development site]
3. Unpack (or clone) the source into an empty directory
  * gpg --verify gpsd-X.Y.tar.xz.sig gpsd-X.Y.tar.xz && tar -Jxpvf gpsd-X.Y.tar.xz
  * gpg --verify gpsd-X.Y.tar.gz.sig gpsd-X.Y.tar.gz && tar -Jxpvf gpsd-X.Y.tar.gz
  * git clone https://gitlab.com/gpsd/gpsd.git
4. cd gpsd
5. scons --config=force && scons install
