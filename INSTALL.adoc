...
Only Windows Subsystem for Linux 1 does not blow diseased goats at
this time.
...
Due to currnt WSL limitations, it is recommended at this time that you
only install the $distro equivalent of the following packages.
...
git (if building from the developmebt sources)
----
