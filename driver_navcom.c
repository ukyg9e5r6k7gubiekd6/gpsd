...
/*    iodc |= getub(buf, 27);*/
    ...
    gpsd_notify(&session->context->errout, LOG_DATA, "Navcom: Transmission time: %u\n", tow);
...
#define LAMBDA_L1 (.190293672798364880476317426464)
	...
	gpsd_notify(&session->context->errout, LOG_DATA + 1,
...
static gps_mask_t handle_0xae(struct gps_device_t *session)
{
    ...
    if (msg_len == 0x0037) {
	gpsd_notify(&session->context->errout, LOG_INF, "Navcom: ID Data: "
			...
			size_t len)
{
    ...
    /*@ -usedef -compdef @*/
    gpsd_notify(&session->context->errout, LOG_RAW, "Navcom: packet type 0x%02x\n", cmd_id);
    /*@ +usedef +compdef @*/
