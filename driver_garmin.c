	...
	, int pkt_len, unsigned char *buf )
{
	...
	if ( 90.0 < session->newdata.latitude ) {
		...
		session->newdata.latitude = -90.0;
		gpsd_report(LOG_INF, "Garmin: ERROR: Latitude negative overrange\n");
	}
	...
	if ( 180.0 < session->newdata.longitude ) {
		...
		session->newdata.longitude = -180.0;
		gpsd_report(LOG_INF, "Garmin: ERROR: Longitude negative overrange\n");
	}
	...
	gpsd_report(LOG_INF, "Garmin: Alt: %.3f, Epe: %.3f, Eph: %.3f, Epv: %.3f, Fix: %d, Gps_tow: %f, Lat: %.3f, Lon: %.3f, LonVel: %.3f, LatVel: %.3f, AltVel: %.3f, MslHgt: %.3f, Leap: %d, GarminDays: %d\n"
	    , pvt->alt
	    , pvt->epe
	    , pvt->eph
	    , pvt->epv
	...
	for ( i = 0, j = 0 ; i < GARMIN_CHANNELS ; i++, sats++ ) {
	    gpsd_report(LOG_INF,"Garmin:   Sat %3d, snr: %5d, elev: %2d, Azmth: %3d, Stat: %x\n"
		, sats->svid
		, sats->snr
		, sats->elev
		, sats->azmth
	...
	break;
    default:
	gpsd_report(LOG_WARN, "Garmin: Unknown packet id: %#02x, Sz: %#02x, pkt:%s\n",
	    pkt_id, pkt_len,
...
// gem
	...
	case GARMIN_PKTID_TRANSPORT_START_SESSION_RESP:
	    ...
	    gpsd_report(LOG_PROG, "Garmin: Transport, Start Session resp, unit: 0x%x\n"
		, mode);
	    break;
	default:
	    gpsd_report(LOG_PROG, "Garmin: Transport, Packet: Type %d %d %d, ID: %d, Sz: %d\n"
			, pkt->mPacketType
			, pkt->mReserved1
			, pkt->mReserved2
			, pkt->mPacketId
	...
	break;
    default:
	gpsd_report(LOG_PROG, "Garmin: Packet: Type %d %d %d, ID: %d, Sz: %d\n"
		    , pkt->mPacketType
		    , pkt->mReserved1
		    , pkt->mReserved2
		    , pkt->mPacketId
...
gps_mask_t garmin_ser_parse(struct gps_device_t *session)
{
    ...
    if ( len < n + i ) {
	Send_NAK();
	gpsd_report(LOG_RAW+1, "Garmin: No checksum, Packet too short %zd < %zd\n",
		    len, n + i);
	return 0;
    }
    c = buf[n + i++];
    ...
    if ( len < n + i ) {
	Send_NAK();
	gpsd_report(LOG_RAW+1, "Garmin: No final DLE, Packet too short %zd < %zd\n",
		    len, n + i);
	return 0;
    }
    c = buf[n + i++];
    ...
    if ( len < n + i ) {
	Send_NAK();
	gpsd_report(LOG_RAW+1, "Garmin: No final ETX, Packet too short %zd < %zd\n",
		    len, n + i);
	return 0;
    }
    c = buf[n + i++];
...
static void garmin_switcher(struct gps_device_t *session, int mode)
{
	...
	if (status == (int)sizeof(switcher)) {
	    gpsd_report(LOG_IO, "Garmin: => GPS: turn off binary %02x %02x %02x... \n"
...
static int GetPacket (struct gps_device_t *session )
{
	...
	if ( 0 >  theBytesReturned ) {
	    ...
	    gpsd_report(LOG_ERROR, "Garmin: GetPacket() read error=%d, errno=%d\n"
	...
	if ( 256 <=  session->driver.garmin.BufferLen ) {
	    ...
	    gpsd_report(LOG_ERROR, "Garmin: GetPacket() packet too long, %ld > 255 !\n"
    ...
    // }
    if ( 10 <= cnt ) {
	    gpsd_report(LOG_ERROR, "Garmin: GetPacket() packet too long or too slow!\n");
	    return -1;
    }
