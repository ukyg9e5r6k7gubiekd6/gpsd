			  ...
			  int pkt_len, unsigned char *buf)
{
    ...
    switch (pkt_id) {
    case ACK:
	gpsd_notify(&session->context->errout, LOG_PROG, "Garmin: ACK\n");
	break;
    case NAK:
	gpsd_notify(&session->context->errout, LOG_PROG, "Garmin: NAK\n");
	break;
...
//
    ...
    gpsd_notify(&session->context->errout, LOG_PROG, "Garmin: PrintUSBPacket()\n");
// gem
...
static void garmin_event_hook(struct gps_device_t *session, event_t event)
{
	...
	gpsd_notify(&session->context->errout, LOG_PROG, "Garmin: Get Product Data\n");
...
gps_mask_t garmin_ser_parse(struct gps_device_t *session)
{
    ...
    gpsd_notify(&session->context->errout, LOG_RAW, "Garmin: garmin_ser_parse()\n");
    if (6 > len) {
	/* WTF? */
	/* minimum packet; <DLE> [pkt id] [length=0] [chksum] <DLE> <STX> */
	Send_NAK();
	gpsd_notify(&session->context->errout, LOG_RAW + 1,
    ...
    if ('\x10' != buf[0]) {
	Send_NAK();
	gpsd_notify(&session->context->errout, LOG_RAW + 1, "Garmin: buf[0] not DLE\n");
	return 0;
    }
    n = 1;
    pkt_id = buf[n++];
	...
	if ('\x10' != buf[n++]) {
	    Send_NAK();
	    gpsd_notify(&session->context->errout, LOG_RAW + 1, "Garmin: Bad pkt_id %#02x\n", pkt_id);
	    return 0;
	}
    }
    ...
    if ('\x10' != c) {
	Send_NAK();
	gpsd_notify(&session->context->errout, LOG_RAW + 1, "Garmin: Final DLE not DLE\n");
	return 0;
    }
    /* get final ETX */
    if (len < n + i) {
    ...
    Send_ACK();
    /*@ +usedef +compdef @*/
    gpsd_notify(&session->context->errout, LOG_DATA, "Garmin: garmin_ser_parse( )\n");
    return mask;
}
...
static int GetPacket(struct gps_device_t *session)
{
    ...
    gpsd_notify(&session->context->errout, LOG_DATA, "Garmin: GetPacket()\n");
    ...
    // gpsd_notify(&session->context->errout, LOG_RAW+1, "Garmin: p[%d] = %x\n", x, session->driver.garmin.Buffer[x]);
    // }
    if (10 <= cnt) {
	gpsd_notify(&session->context->errout, LOG_ERROR,
...
{
    gpsd_notify(&session->context->errout,  LOG_PROG, "Garmin: garmin_usb_parse()\n");
