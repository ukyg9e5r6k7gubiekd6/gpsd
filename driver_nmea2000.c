		       ...
		       unsigned char *buffer, int len, PGN *pgn)
{
	...
	ptr += l2;
        for (l1=0;l1<len;l1++) {
            if (((l1 % 20) == 0) && (l1 != 0)) {
	        gpsd_notify(&context->errout, LOG_IO,"%s\n", bu);
		ptr = 0;
                l2 = sprintf(&bu[ptr], "                   : ");
		ptr += l2;
            }
...
int nmea2000_open(struct gps_device_t *session)
{
	...
	for (l = 0; l < NMEA2000_NETS; l++) {
	    if (strncmp(can_interface_name[l], 
			interface_name,
			MIN(sizeof(interface_name), sizeof(can_interface_name[l]))) == 0) {
	        gpsd_notify(&session->context->errout, LOG_ERROR, "NMEA2000 open: CAN device duplicate open: %s .\n", interface_name);
		return -1;
	    }
	}
	for (l = 0; l < NMEA2000_NETS; l++) {
    ...
    if (status != 0) {
        gpsd_notify(&session->context->errout, LOG_ERROR, "NMEA2000 open: bind failed.\n");
	close(sock);
	return -1;
    }
